...
const EXAMPLE_CONTROLLERS = [
  {
    name: 'P Controller',
    latex: 'K(s) = K_\\mathrm{P}',
    code: 'Kp = 1\nK = Kp'
  },
  {
    name: 'PI Controller',
    latex: 'K(s) = K_\\mathrm{P}\\left(1 + \\frac{1}{T_\\mathrm{I} s}\\right)',
    code: 'Kp = 1\nTi = 0.5\nK = Kp * (1 + 1/(Ti*s))'
  },

  {
    name: 'PD Controller (Ideal)',
    latex: 'K(s) = K_\\mathrm{P} (1 + T_\\mathrm{D} s)',
    code: 'Kp = 1\nTd = 0.1\nK = Kp * (1 + Td*s)'
  },
  {
    name: 'PID Controller (Ideal)',
    latex: 'K(s) = K_\\mathrm{P} \\left(1 + \\frac{1}{T_\\mathrm{I} s} + T_\\mathrm{D} s\\right)',
    code: 'Kp = 1\nTi = 2\nTd = 0.1\nK = Kp * (1 + 1/(Ti*s) + Td*s)'
  },

  {
    name: 'PD Controller (with roll-off)',
    latex: 'K(s)=K_\\mathrm{P}\\left(1+\\frac{T_\\mathrm{D} s}{1+\\frac{T_\\mathrm{D}}{N}s}\\right)',
    code: 'Kp = 1\nTd = 0.1\nN = 10\nK = Kp * (1 + (Td*s)/(1 + (Td/N)*s))'
  },
  {
    name: 'PID Controller (with roll-off)',
    latex: 'K(s)=K_\\mathrm{P}\\left(1+\\frac{1}{T_\\mathrm{I} s}+\\frac{T_\\mathrm{D} s}{1+\\frac{T_\\mathrm{D}}{N}s}\\right)',
...
const EXAMPLE_PLANTS = [
...

  {
    ...
    latex: 'P(s) = \\frac{1}{Ts + 1} e^{-L_\\mathrm{d} s}',
    code: 'T = 1\nLd = 0.5\nP = 1 / (T*s + 1) * exp(-Ld*s)'
  },

  {
    name: 'Delay (Padé 1,1)',
    latex: 'e^{-L_\\mathrm{d} s}\\approx\\frac{1-\\frac{L_\\mathrm{d}}{2}s}{1+\\frac{L_\\mathrm{d}}{2}s}',
    code: 'Ld = 0.5\nD = (1 - (Ld/2)*s) / (1 + (Ld/2)*s)'
  },
  {
    name: 'First-order + Delay (Padé 1,1)',
    latex: 'P(s)=\\frac{1}{Ts+1}\\,\\frac{1-\\frac{L_\\mathrm{d}}{2}s}{1+\\frac{L_\\mathrm{d}}{2}s}',
    code: 'T = 1\nLd = 0.5\nD = (1 - (Ld/2)*s) / (1 + (Ld/2)*s)\nP = 1 / (T*s + 1) * D'
  },

  {
    name: 'Delay (Padé 2,2)',
    latex: 'e^{-L_\\mathrm{d} s}\\approx\\frac{1-\\frac{L_\\mathrm{d}}{2}s+\\frac{(L_\\mathrm{d} s)^2}{12}}{1+\\frac{L_\\mathrm{d}}{2}s+\\frac{(L_\\mathrm{d} s)^2}{12}}',
    code: 'Ld = 0.5\nD = (1 - (Ld/2)*s + (Ld^2/12)*s^2) / (1 + (Ld/2)*s + (Ld^2/12)*s^2)'
  },
  {
    name: 'Delay (Padé n,m)',
    latex: 'e^{-L_\\mathrm{d} s}\\approx\\text{pade\\_delay}(L_\\mathrm{d},n,m)',
    code: 'Ld = 0.5\nD = pade_delay(Ld, 3, 3)'
  },

  {
    name: 'Second-order System',
    latex: 'P(s) = \\frac{\\omega_\\mathrm{n}^2}{s^2 + 2\\zeta\\omega_\\mathrm{n} s + \\omega_\\mathrm{n}^2}',
...

  {
    ...
    latex: 'P(s) = \\frac{1 - T_\\mathrm{z} s}{(T_1 s + 1)(T_2 s + 1)}',
...
const EXAMPLE_FILTERS = [
  ...
  },
  {
    ...
    latex: 'F(s)=\\frac{\\omega_\\mathrm{n}^2}{s^2+2\\zeta\\omega_\\mathrm{n} s+\\omega_\\mathrm{n}^2}',
  ...
  },
  {
    ...
    latex: 'F(s)=\\frac{2\\zeta\\omega_\\mathrm{n} s}{s^2+2\\zeta\\omega_\\mathrm{n} s+\\omega_\\mathrm{n}^2}',
    code: 'wn = 10\nzeta = 0.5\nF = 2*zeta*wn*s / (s^2 + 2*zeta*wn*s + wn^2)'
  },
  {
    name: 'Notch Filter',
    latex: 'F(s)=\\frac{s^2+\\omega_\\mathrm{n}^2}{s^2+2\\zeta\\omega_\\mathrm{n} s+\\omega_\\mathrm{n}^2}',
    code: 'wn = 10\nzeta = 0.1\nF = (s^2 + wn^2) / (s^2 + 2*zeta*wn*s + wn^2)'
  },
  {
    name: 'Derivative Roll-off (Washout / D-filter)',
    latex: 'F(s)=\\frac{T_\\mathrm{D} s}{1+\\frac{T_\\mathrm{D}}{N}s}',
    code: 'Td = 0.05\nN = 10\nF = (Td*s) / (1 + (Td/N)*s)'
  },
  {
    name: 'Moving-average (delay average)',
    latex: 'F(s)=\\frac{1-e^{-L_\\mathrm{d} s}}{L_\\mathrm{d} s}',
    code: 'Ld = 0.1\nF = (1 - exp(-Ld*s)) / (Ld*s)'
  }
];
